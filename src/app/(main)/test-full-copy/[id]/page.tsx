        ...
        return (
          <div className="p-6">
            ...
            <div className="mb-6 p-4 border-2 border-dashed border-green-500 bg-green-50 rounded-lg">
              <h3 className="font-bold text-green-700 mb-2">测试文件上传（完整复制）</h3>
              <input 
                type="file" 
                onChange={(e) => {
                  console.log('完整复制 - 文件选择成功:', e.target.files?.[0]?.name);
                  alert('完整复制 - 文件选择成功: ' + e.target.files?.[0]?.name);
                }}
              ...
              {navigationState && (
                <>
                  ...
                  <div className="text-sm text-slate-500 px-2 min-w-[120px] text-center">
                    {navigationState.currentIndex + 1} / {navigationState.caseIds?.length || 0}
                  </div>
                  <Button
                    variant="outline"
                    size="icon"
