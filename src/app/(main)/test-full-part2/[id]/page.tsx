    ...
    switch (activeTab) {
      case 'legal':
        return (
          <div className="p-6">
            {/* 测试用最简单的文件上传 */}
            <div className="mb-6 p-4 border-2 border-dashed border-purple-500 bg-purple-50 rounded-lg">
              <h3 className="font-bold text-purple-700 mb-2">测试文件上传（完整版中间部分）</h3>
              <input 
                type="file" 
                onChange={(e) => {
                  console.log('完整版中间部分 - 文件选择成功:', e.target.files?.[0]?.name);
                  alert('完整版中间部分 - 文件选择成功: ' + e.target.files?.[0]?.name);
                }}
